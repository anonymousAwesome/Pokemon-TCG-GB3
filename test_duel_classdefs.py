...
def setup_duel():
    ...
    deck_objects1=decks.generate([[cards.dratini,1],[cards.hitmonchan,1],[cards.seel,1],[cards.machop,1],[cards.water,7]])
    deck_objects2=decks.generate([[cards.dratini,1],[cards.hitmonchan,1],[cards.seel,1],[cards.machop,1],[cards.water,7]])
    player1.deck.cards=deck_objects1
    player2.deck.cards=deck_objects2
    cd.move_cards_to_from(cd.Pokemon(**cards.seel),player1.active)
    cd.move_cards_to_from(cd.Pokemon(**cards.seel),player2.active)
    player1.place_prizes()
    player2.place_prizes()
    return duel_manager, player1, player2


def test_one_active_pokemon(pikachu1):
    active=cd.CardCollection(pikachu1)
    assert pikachu1 in active

def test_two_active_pokemon(pikachu1,pikachu2):
    active1=cd.CardCollection(pikachu1)
    active2=cd.CardCollection(pikachu2)
    assert pikachu1 in active1
    assert pikachu2 in active2

def test_one_pokemon_attacking_another(player1,pikachu1,pikachu2):
    pikachu1.attack(player1,pikachu2,0)
    assert pikachu2.hp==30

def test_hp_past_max_doesnt_go_negative(player1,pikachu1,pikachu2):
    pikachu1.attacks[0]["damage"]=50
    pikachu1.attack(player1,pikachu2,0)
    assert pikachu2.hp==0

def test_negative_damage(player1,pikachu1,pikachu2):
...
def test_starting_duel_phase():
    ...
    assert main.phase_handler.get_game_phase()=="duelling"
...
def test_attach_energy_to_pokemon(player1,pikachu1):
    water_energy=cd.Energy("water", "basic energy", "energy")
    pikachu1.attach_card(water_energy)
    assert water_energy in pikachu1.attached

def test_moving_water_deck_hand_discard():
    deck=cd.CardCollection()
    hand=cd.CardCollection()
    discard_pile=cd.CardCollection()
    water_energy=cd.Energy("water", "basic energy","energy")
...
def test_moving_pikachu_deck_hand_bench_active(pikachu1):
    deck=cd.CardCollection()
    hand=cd.CardCollection()
    active=cd.CardCollection()
    bench=cd.CardCollection()
...
def test_evolve_pikachu(pikachu1, raichu1):
    active=cd.CardCollection()
...
def test_weakness(player1,pikachu1):
    geodude = cd.Pokemon(
        name="Geodude",
        cardset="base",
        level_id=999,
    ...
    geodude.attack(player1,pikachu1,0)
    assert pikachu1.hp==0

def test_load_cards_from_file():
    dratini=cd.Pokemon(**cards.dratini)
...
def test_deck_shuffle(monkeypatch):
    ...
    duel_manager=cd.DuelManager(prizes=6)
    player1=cd.Player(duel_manager)
    pkmn_card_1=cd.Pokemon(**cards.dratini)
    pkmn_card_2=cd.Pokemon(**cards.seel)
    pkmn_card_3=cd.Pokemon(**cards.machop)
    deck = cd.CardCollection(cards=[pkmn_card_1, pkmn_card_2, pkmn_card_3])
...
'''
    duel_manager=cd.DuelManager(prizes=1)
    player1=cd.Player(duel_manager)
    player2=cd.Player(duel_manager)
    for i in range(2):
        cd.move_cards_to_from(cd.Pokemon(**cards.seel),player1.deck)
    for i in range(8):
        cd.move_cards_to_from(cd.Energy("water energy", "base","energy"),player1.deck)
    for i in range(2):
        cd.move_cards_to_from(cd.Pokemon(**cards.voltorb),player2.deck)
    for i in range(8):
        cd.move_cards_to_from(cd.Energy("lightning energy", "base","energy"),player2.deck)
...
def test_attacking_with_secondary_attack():
    ...
    player1 = cd.Player(duel_manager)
    player2 = cd.Player(duel_manager)
    cd.move_cards_to_from(cd.Pokemon(**cards.hitmonchan),player1.active)
    cd.move_cards_to_from(cd.Pokemon(**cards.hitmonchan),player2.active)
    player1.active[0].attack(player1,player2.active[0],1)
...
def test_initial_draw(monkeypatch):
    ...
    player1 = cd.Player(duel_manager)
    monkeypatch.setattr(random, 'shuffle', lambda x: x.reverse())
    player1.deck.cards=decks.generate([[cards.dratini,1],[cards.hitmonchan,1],[cards.seel,1],[cards.machop,1],[cards.water,7]])
